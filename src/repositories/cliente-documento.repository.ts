...
import { clienteDocumentos, ClienteDocumentoDTO } from "@model/tables/cliente-documento.model";
...
export class ClienteDocumentoRepository {
  ...
  async findAllPaginated(
    ...
    if (filters?.fechaInicio && filters?.fechaFin) {
      conditions.push(
        gte(clienteDocumentos.creadoEn, new Date(filters.fechaInicio)),
      );
      conditions.push(
        lte(clienteDocumentos.creadoEn, new Date(filters.fechaFin + "T23:59:59")),
      );
