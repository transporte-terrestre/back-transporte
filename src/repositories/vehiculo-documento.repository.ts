...
import { vehiculoDocumentos, VehiculoDocumentoDTO } from "@model/tables/vehiculo-documento.model";
...
export class VehiculoDocumentoRepository {
  ...
  async findAllPaginated(
    ...
    if (filters?.fechaInicio && filters?.fechaFin) {
      conditions.push(
        gte(vehiculoDocumentos.creadoEn, new Date(filters.fechaInicio)),
      );
      conditions.push(
        lte(vehiculoDocumentos.creadoEn, new Date(filters.fechaFin + "T23:59:59")),
      );
